...
def plot_raw_timestamps(csv_files, output_dir):
    ...
    for diff_name, (start_col, end_col) in time_diff_pairs.items():
        time_diffs[diff_name] = calculate_time_difference(combined_df, start_col, end_col)
    
    # Extract other metrics directly from the dataframe
    if 'batch_time_ns' in combined_df.columns:
        # Convert from ns to ms
        time_diffs['batch_time'] = combined_df['batch_time_ns'].to_numpy() / 1e6
    ...
    if 'iterations' in combined_df.columns and 'batch_time_ns' in combined_df.columns:
        ...
        time_diffs['efficiency'] = iterations / (combined_df['batch_time_ns'].to_numpy() / 1e6)
...
def plot_batch_size_comparison(threading_types, reactive_types, batch_sizes, num_runs, output_dir):
    ...
    for threading in threading_types:
        for reactive in reactive_types:
            ...
            for batch_size in batch_sizes:
                ...
                pattern = f"results/anytime_raw_timestamps_batch_{batch_size}_{reactive}_{threading}_run*.csv"
                matching_files = glob.glob(pattern)
                
                if not matching_files:
                    # Try the old timing data pattern as fallback
                    pattern = f"results/anytime_timing_data_batch_{batch_size}_{reactive}_{threading}_run*.csv"
                    matching_files = glob.glob(pattern)
                ...
                print(f"Found {len(matching_files)} files for batch_size={batch_size}")
                ...
                print(f"Combined {len(dfs)} runs with total {len(combined_df)} rows for batch_size={batch_size}")
                ...
                for diff_name, (start_col, end_col) in time_diff_pairs.items():
                    if start_col in combined_df.columns and end_col in combined_df.columns:
                        metrics[diff_name] = calculate_time_difference(combined_df, start_col, end_col)
                
                # Extract additional metrics
                if 'batch_time_ns' in combined_df.columns:
                    metrics['batch_time'] = combined_df['batch_time_ns'].to_numpy() / 1e6
                ...
                if 'iterations' in combined_df.columns and 'batch_size' in combined_df.columns:
                    metrics['iterations_per_batch'] = combined_df['iterations'].to_numpy() / combined_df['batch_size'].to_numpy()
                
                if 'iterations' in combined_df.columns and 'batch_time_ns' in combined_df.columns:
                    metrics['efficiency'] = combined_df['iterations'].to_numpy() / (combined_df['batch_time_ns'].to_numpy() / 1e6)
    ...
    def create_boxplots_for_metrics(metrics_list, is_latency_metric=False):
        ...
        for metric_name in metrics_list:
                ...
                for config, metrics in all_data.items():
                    if metric_name in metrics and batch_size in metrics[metric_name]:
                        ...
                        if len(data) > 0:
                            ...
                            config_stderrs[config][batch_size] = np.std(data) / np.sqrt(len(data))
            ...
            plt.title(f'{metric_name.replace("_", " ").title()} vs Batch Size - All Configurations (Combined Runs)')
...
def main():
    ...
    for threading in args.threading:
        for reactive in args.reactive:
            for batch_size in args.batch_sizes:
                ...
                file_pattern = f"results/anytime_raw_timestamps_batch_{batch_size}_{reactive}_{threading}_run1.csv"
    ...
    plot_batch_size_comparison(args.threading, args.reactive, args.batch_sizes, args.runs, output_dir)
