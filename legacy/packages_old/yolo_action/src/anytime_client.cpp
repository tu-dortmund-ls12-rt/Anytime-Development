...
AnytimeActionClient::AnytimeActionClient(const rclcpp::NodeOptions& options)
  ...
  timer_ = this->create_wall_timer(std::chrono::milliseconds(200),
                                   [this]() { this->send_goal(); });
  goal_handle_timer_ = this->create_wall_timer(
      std::chrono::seconds(0), [this]() { this->receive_goal_handle(); });
  goal_handle_timer_->cancel();
  result_timer_ = this->create_wall_timer(std::chrono::seconds(0),
                                          [this]() { this->receive_result(); });
  result_timer_->cancel();
  cancel_timer_ = this->create_wall_timer(std::chrono::seconds(0),
                                          [this]() { this->cancel_goal(); });
  cancel_timer_->cancel();
  cancel_timeout_timer_ = this->create_wall_timer(
      std::chrono::seconds(3), [this]() { this->cancel_timeout_callback(); });
...
void AnytimeActionClient::receive_goal_handle() {
  ...
  if (goal_handle_future_.wait_for(std::chrono::seconds(0)) ==
    ...
    RCLCPP_INFO(this->get_logger(),
                "Goal accepted by server, sending result request");
...
void AnytimeActionClient::receive_result() {
  ...
  if (result_future_.wait_for(std::chrono::seconds(0)) ==
    ...
    rclcpp_action::ClientGoalHandle<
        anytime_interfaces::action::Anytime>::WrappedResult wrapped_result =
        result_future_.get();
    switch (wrapped_result.code) {
      ...
      case rclcpp_action::ResultCode::CANCELED:
        ...
        RCLCPP_INFO(this->get_logger(), "Result after cancel: %f",
                    wrapped_result.result->result);
        break;
      default:
    ...
    cancel_future_ = action_client_->async_cancel_goal(
        goal_handle_,
        [this](std::shared_ptr<action_msgs::srv::CancelGoal::Response>
                   cancel_result) {
          this->cancel_response_callback(cancel_result);
        });
    // cancel_timer_->reset();
    return;
  }
...
void AnytimeActionClient::cancel_response_callback(
    std::shared_ptr<action_msgs::srv::CancelGoal::Response> cancel_result) {
  if (cancel_result->return_code ==
...
void AnytimeActionClient::cancel_goal() {
  ...
  if (cancel_future_.wait_for(std::chrono::seconds(0)) ==
      std::future_status::ready) {
    // print the value of the result from the goal handle
    //  RCLCPP_INFO(this->get_logger(), "Result received: %f",
    //  cancel_future_.get()->result->result);
