...
class CudaBuffer {
  public:
    ...
    CudaBuffer(const CudaBuffer& other) : device_ptr(nullptr), size(0) {
        if (other.size > 0) {
            ...
            if (allocate(other.size)) {
                ...
                cudaError_t status = cudaMemcpy(device_ptr, other.device_ptr, size, 
                                               cudaMemcpyDeviceToDevice);
                if (status != cudaSuccess) {
                    std::cerr << "Error copying data in copy constructor: " 
            ...
            if (other.size > 0) {
                if (allocate(other.size)) {
                    cudaError_t status = cudaMemcpy(device_ptr, other.device_ptr, size, 
                                                   cudaMemcpyDeviceToDevice);
                    if (status != cudaSuccess) {
                        std::cerr << "Error copying data in assignment operator: " 
                                  << cudaGetErrorString(status) << std::endl;
                        free();  // Clean up if copy failed
                    }
                }
    ...
    bool allocate(size_t size) {
        ...
        if (status != cudaSuccess) {
            std::cerr << "Error allocating CUDA memory: "
                      << cudaGetErrorString(status) << std::endl;
        }
    ...
    for (int c = 0; c < channels; c++) {
        for (int h = 0; h < height; h++) {
            for (int w = 0; w < width; w++) {
                nchw_data[c * height * width + h * width + w] =
...
bool loadImageFromFile(const std::string &imagePath, CudaBuffer &buffer,
...
bool saveEngineToFile(const std::string &enginePath,
                      const IHostMemory &serializedEngine) {
    std::ofstream engineFile(enginePath, std::ios::binary);
    if (!engineFile) {
        std::cerr << "Failed to open file for writing: " << enginePath
                  << std::endl;
        return false;
    }
    ...
    std::cout << "Engine successfully serialized to: " << enginePath
              << std::endl;
    return true;
}

bool buildOnnxEngine(const std::string &onnx_path,
    ...
    const std::unique_ptr<INetworkDefinition> network(
    ...
    const std::unique_ptr<IBuilderConfig> config(
...
bool loadEngine(const std::string &enginePath,
    ...
    if (!engineFile) {
        std::cerr << "Failed to open file for reading: " << enginePath
                  << std::endl;
        return false;
    }
    ...
    if (engineFile.fail()) {
        std::cerr << "Error occurred while reading engine from file"
                  << std::endl;
        return false;
    }
    engineFile.close();
    ...
    engine = std::unique_ptr<ICudaEngine>(
        runtime->deserializeCudaEngine(engineData.get(), engineSize));
    if (!engine) {
        return false;
    }
...
class InferenceState {
  public:
    ...
    InferenceState(const CudaBuffer &inputBuffer, cudaStream_t stream,
                   bool halfPrecision, int chunkCount,
                   int exitIndexToProcess = 0)
        : currentStage(LAYER_PROCESSING),
          currentIndex(0) {

        this->inputBuffer.allocate(inputBuffer.getSize());
        this->inputBuffer.copyFromHost(inputBuffer.getDevicePtr(),
                                       inputBuffer.getSize());
...
class AnytimeYOLO {
  public:
    AnytimeYOLO(const std::string &folderPath, bool halfPrecision = false) {
        ...
        std::cout << "Loaded config file: " << folderPath + "/model.json"
                  << std::endl;
        std::cout << "Loading layers..." << std::endl;

        for (const auto &layerConfig : config["layers"]) {
            std::cout << "Loading layer: " << layerConfig["index"] << " - "
                      << layerConfig["type"] << std::endl;
        ...
        if (loadEngine(nmsEnginePath, runtime, nmsEngine)) {
            std::cout << "Successfully loaded cached NMS engine from: "
                      << nmsEnginePath << std::endl;
        } else {
            std::cerr
                << "Failed to load cached NMS engine, falling back to build"
                << std::endl;
    ...
    BindingInfo getBindingInfo(const nvinfer1::ICudaEngine *engine) {
        ...
        for (int b = 0; b < numBindings; b++) {
            ...
            if (engine->getTensorIOMode(name) ==
    ...
    OutputInfo getOutputInfo(const nvinfer1::ICudaEngine *engine,
        ...
        nvinfer1::DataType outType =
            engine->getTensorDataType(outputName.c_str());
    ...
    bool
    executeEngine(const nvinfer1::ICudaEngine *engine,
    ...
    bool processChunk(const AnytimeYOLOChunk &chunk,
                      const CudaBuffer &inputBuffer,
                      const std::vector<CudaBuffer> &prevLayerOutputs,
                      CudaBuffer &outputBuffer, std::vector<int> &outputDims,
                      cudaStream_t stream,
            ...
            if (isLayer) {
                ...
                if (f == -1) {
                    if (chunk.index == 0) {
                        ...
                        inputPtr =
                            prevLayerOutputs[chunk.index - 1].getDevicePtr();
                    }
                } else
        ...
        // Get output info
        auto outputInfo =
            getOutputInfo(chunk.engine.get(), bindingInfo.outputNames[0]);
        outputDims = outputInfo.dimensions;

        // Allocate output buffer
        if (!outputBuffer.allocate(outputInfo.bufferSize)) {
            std::cerr << "Failed to allocate " << (isLayer ? "layer" : "exit")
        ...
        // Execute the node
        if (!executeEngine(chunk.engine.get(), chunk.context.get(), inputs,
                           outputs, stream)) {
            std::cerr << (isLayer ? "Layer" : "Exit") << " execution failed"
                      << std::endl;
            return false;
        }
    ...
    bool processNMS(const CudaBuffer &exitOutputBuffer,
                    CudaBuffer &nmsOutputBuffer, cudaStream_t stream) {

        // Get binding information
        auto bindingInfo = getBindingInfo(nmsEngine.get());

        // Fixed size for NMS output (as in original code)
        size_t nmsOutputSize =
            6 * 1000 * sizeof(float); // Assuming 1000 detections
        ...
        auto nmsContext = std::unique_ptr<IExecutionContext>(
            nmsEngine->createExecutionContext());
        // Execute NMS
        std::cout << "Input pointer " << inputs[0].second << std::endl;

        if (!executeEngine(nmsEngine.get(), nmsContext.get(), inputs, outputs,
                           stream)) {
    ...
    void dump_output(const CudaBuffer &outputBuffer,
        ...
        if (!outputBuffer.copyToHost(host_data.data(),
    ...
    CudaBuffer convertToFullPrecision(const CudaBuffer &inputBuffer) {
        ...
        std::vector<float> fullPrecisionData(inputBuffer.getSize() /
                                             sizeof(__half));
        ...
        fullPrecisionBuffer.copyFromHost(fullPrecisionData.data(),
                                         fullPrecisionBuffer.getSize());

        return fullPrecisionBuffer;
    }

    // Main inference function
    std::vector<std::vector<float>> infer(const CudaBuffer &inputBuffer,
        ...
        for (size_t i = 0; i < chunks.size(); i++) {
            if (!processChunk(chunks[i], inputBuffer, layerOutputBuffers,
                              layerOutputBuffers[i], outputDims[i], stream)) {
        ...
        for (size_t i = 0; i < 1; i++) {
            if (!processChunk(exits[i], inputBuffer, layerOutputBuffers,
                              exitOutputBuffers[i], exitOutputDims[i], stream,
                              false)) {
        ...
        for (size_t i = 0; i < 1; i++) {
            ...
            if (halfPrecision)
                input = convertToFullPrecision(exitOutputBuffers[i]);
            ...
            if (!nmsOutputBuffers[i].copyToHost(finalOutputs[i].data(),
                                                nmsOutputSize)) {
    ...
    InferenceState createInferenceState(const CudaBuffer &inputBuffer) {
        return InferenceState(inputBuffer, stream, halfPrecision,
                              chunks.size());
    }
    ...
    bool inferStep(InferenceState &state, bool async = true) {
        ...
        case InferenceState::LAYER_PROCESSING: {
            if (state.currentIndex < chunks.size()) {
                ...
                std::cout << "Processing layer " << state.currentIndex
                          << std::endl;
                // Process this layer
                if (!processChunk(
                        chunk, state.inputBuffer, state.layerOutputBuffers,
                        state.layerOutputBuffers[state.currentIndex],
                        state.outputDims[state.currentIndex], stream)) {
                    throw std::runtime_error(
                        "Layer processing failed at index " +
                        std::to_string(state.currentIndex));
                }
        ...
        case InferenceState::EXIT_PROCESSING: {
            ...
            if (!processChunk(exit, state.inputBuffer, state.layerOutputBuffers,
                              state.exitOutputBuffer, state.exitOutputDims,
                              stream, false)) {
                throw std::runtime_error("Exit processing failed");
            }
        ...
        case InferenceState::NMS_PROCESSING: {
            ...
            for (size_t i = 0; i < 1; i++) {
                ...
                if (!nmsOutputBuffers[i].copyToHost(finalOutputs[i].data(),
                                                    nmsOutputSize)) {
                    throw std::runtime_error(
                        "Failed to copy NMS output to host");
                }
            }
    ...
    std::vector<float> finishEarly(InferenceState &state) {
        ...
        if (!processChunk(exit, state.inputBuffer, state.layerOutputBuffers,
                          state.exitOutputBuffer, state.exitOutputDims, stream,
                          false)) {
            throw std::runtime_error("Exit processing failed");
        }
        ...
        if (!nmsOutputBuffer.copyToHost(state.finalOutput.data(),
                                        nmsOutputSize)) {
            throw std::runtime_error("Failed to copy NMS output to host");
        }
  ...
  private:
    ...
    bool loadLayer(const json &layerConfig) {
        ...
        const std::string enginePath =
            weightsPath.substr(0, weightsPath.find_last_of('.')) + ".engine";
        std::cout << "Engine path: " << enginePath << std::endl;

        std::unique_ptr<ICudaEngine> engine;
        if (loadEngine(enginePath, runtime, engine)) {
            std::cout << "Successfully loaded cached engine from: "
                      << enginePath << std::endl;
        } else {
            std::cerr << "Failed to load cached engine, falling back to build"
                      << std::endl;
        ...
        chunks.push_back(
            {type, index, f, std::move(engine), std::move(context)});
        return true;
    }
